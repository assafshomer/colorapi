...
var properties = {}
properties.colu_sdk = {}
properties.colu_sdk.host = 'http://localhost'
...
Colu.prototype.genericIssue = function (publicKey, assetname, assetshortname, amount, fee, selfhost, devis, image, icon, version, type, desc, usermeta, callback) {
  assert(typeof devis === 'number' && parseInt(devis, 10) === devis && devis >= 0 && devis <= 8, 'Devisibility must be an integer number between 0 and 8')
  amount = '' + amount
  type = '' + type
  desc = '' + desc
  assetname = '' + assetname
...
Colu.prototype.accessCCIssue = function (publicKey, toAddress, txid, vout, callback) {
  ...
  return self.ccIssue(options, callback)
}
